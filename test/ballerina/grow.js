...
var world = "./Grow/world4.grw";
var imageLocation = "./Grow/images/";

var playerColor = "#ff0000";
var shipColor = "#bbbb77";
...
function isClear(e){
	try{
		var x = parseInt(e.clientX/particleSize);
		var y = parseInt(e.clientY/particleSize);
...
function reset(){
	...
	particles = [];
	modPart = new Array(parseInt(window.innerWidth/particleSize + 1)*parseInt(window.innerHeight/particleSize + 1));
	modInd = 0;
	updatedInd = 0;
	updatedPart = new Array(parseInt(window.innerWidth/particleSize+1));
	for(var i=0;i<parseInt(window.innerWidth/particleSize + 1);i++){
		updatedPart[i] = new Array(parseInt(window.innerHeight/particleSize + 1));
...
function init(){
	...
	body.addEventListener('mousemove', function(event){
		curX=event.clientX;
		curY=event.clientY;
		var ben = getBeneath(event);
		if(ben!=null){
			if(ben.nodeName=="A")
				canvas.style.cursor="pointer";
			else
				canvas.style.cursor=style(ben).cursor;
		} else if(ben!=null)
			canvas.style.cursor = ben.style.cursor;
		else
			canvas.style.cursor="default";
		if(isDown)
			mouseMove(event);
	}, false);
	body.onkeydown = function(event){if(!event)var event=window.event;event.cancelBubble=true;event.stopPropagation();key(event); return false;}/*, true)*/;
	body.addEventListener('keyup', function(event){gen = event.shiftKey;keyUp(event); if(holdForElement){elemType="droplet";}}, false);
	body.addEventListener('touchmove', function(){touchMove(event);}, false);
		...
		while(txt != "" && done){
			...
			if(tagType=="elem"){
				if(positioning=="relative"){
					if(shape=="rectangle")
						rectanglePoint(parseInt(partWidth*x), parseInt(partHeight*y), partWidth*width, partHeight*height, type, true, []);
					else if(shape=="circle")
						circlePoint(parseInt(partWidth*x), parseInt(partHeight*y), parseInt(partWidth*width), type, true, []);
					else if(shape=="elipse")
						elipsePoint(parseInt(partWidth*x), parseInt(partHeight*y), parseInt(partWidth*width), parseInt(partHeight*height), type, true, []);
				...
				if(positioning=="relative"){
					player.x = parseInt(partWidth*x);
					player.y = parseInt(partHeight*y);
				} else if(positioning=="absolute"){
					player.x = parseInt(x);
					player.y = parseInt(y);
				}
	...
	genImage.src = imageLocation + "gen.jpg";
...
function key(e){
	...
	if(code==9) /*tab*/
		...
		if((player.jump==0 && !player.hasJumped) || shipMode){
	...
	if(code==17 && showPlayer) /*CTRL key*/
		...
		alert("updatedPart: " + updatedPart[parseInt(curX/particleSize)][parseInt(curY/particleSize)] + "\npariticles: " + particles[parseInt(curX/particleSize)][parseInt(curY/particleSize)].fresh);
		var x = parseInt(curX/particleSize);
		var y = parseInt(curY/particleSize);
			var count=0;
			for(var i=-1;i<2;i++)
				for(var j=-1;j<2;j++)
...
function keyUp(e){
	...
	if(code==32){
		ship = false;
		forcePoint(player.x, player.y+1, particles[player.x][player.y+1].fresh);
	}
	else if(code==37 || code==39)
		player.move = 0;
}

/*Particulate class definition*/
...
function touchMove(evt) {
	try{
		...
		for(var i=0;i<touches.length;i++){
			circlePoint(parseInt(touches[i].clientX/particleSize), parseInt(touches[i].clientY/particleSize), penSize, getElem(), true, []);
		}
	} catch(e){}
}
...
function mouseMove(event){
	if(event.clientX!=0 || event.clientY!=0){
		if(playerMode){
			...
			do{
				circlePoint(parseInt(x/particleSize), parseInt(y/particleSize), penSize, getElem(), true, []);
...
function move(){
	var x = parseInt(prevX)/particleSize;
	var y = parseInt(prevY)/particleSize;
	var magnitude = Math.sqrt(Math.pow(x-player.x,2) + Math.pow(y-player.y,2));
	...
	circlePoint(parseInt(x), parseInt(y), penSize, getElem(), true, []);
}
...
function updateAction(x, y){
	try{if(particles[x][y]==null)return false;}catch(e){return false;} /*Check to make sure that it's a good particle*/
	...
	if(elem=="empty" || elem=="wall" || elem=="growth" || elem=="ice" || elem=="explosive" || elem=="bark" || elem=="cloak")
		...
		if(move<1){
			...
			updateAction(move+x, y+1, "lightning");
		}
		else{
			changePoint(x, y+1, "lightning");
			updateAction(x, y+1, "lightning");
		}
...
function draw(){
	...
	if(showPlayer){
		...
		if(ship){
			point.fillStyle = shipColor;
			point.fillRect(player.x*particleSize, (player.y+1)*particleSize, particleSize, particleSize);
		}
...
function updatePlayer(){
	...
	for(var i=0;i<numBullets;i++){
		...
		try{
			if(isSolid(particles[x][y].fresh) || particles[x][y].fresh == "sticky" || getDensity(particles[x][y].fresh)>getDensity("droplet")){
				...
				x = Math.ceiling(bullet.x);
				y = Math.ceiling(bullet.y);
				if(isSolid(particles[x][y].fresh) || particles[x][y].fresh == "sticky" || getDensity(particles[x][y].fresh)>getDensity("droplet")){
					circlePoint(x, y, bulletExplosionRadius, bullet.elem, true, ["growth","explosive","liquidExplosive", "cloak"]);
					player.bullets[i] = new Bullet();
				}
	...
	/* Update tractor field */
	if(ship)
		for(var i=-1*tractorStrength;i<tractorStrength;i++){
			for(var j=-1*tractorStrength;j<tractorStrength;j++){
				var dist = Math.sqrt(Math.pow(i,2)+Math.pow(j,2));
				if(dist<=tractorStrength){
					var dx=0, dy=0;
					if(dist>=tractorStrength-2){
						dy = 2;
						dx = 1;
					}
					else
					...
					if(particles[player.x+i][player.y+j]){
						var elem = particles[player.x+i][player.y+j].fresh;
						if((isLiquid(elem) || isGas(elem)) && Math.random()*tractorStrength>dist){
							forcePoint(player.x+i, player.y+j, "empty");
							changePoint(player.x+i+dx, player.y+j+dy, elem);
						}
					}
				}
			}
...
/* Get computed style so I can use stylesheets */
function style(dom){
	var s;
